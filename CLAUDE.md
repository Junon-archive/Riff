...
- 악보 렌더: 지판 = **자체 SVG**(`render/fretboard.ts`). **모든 타브 = 오선보(+타브 결합, 박자 공유) VexFlow**(`render/staff.ts`, `meta.notation:"staff"|"staff+tab"|"rhythm"`, 미지정은 staff+tab 폴백). VexFlow는 **빌드타임에만**(jsdom devDep) SVG 생성 → 클라이언트 JS 0. `renderScore()`가 `type`/`meta.notation`으로 분기. (구 자체 타브 SVG `tab.ts` 는 백로그 18 에서 제거 — 전 커리큘럼 staff+tab 이식 완료.) AlphaTab 미채택(재생 필요 시 자체 Web Audio).
