...
## 업데이트 백로그로 라우팅
- `_design_docs/05_update_backlog/` 는 미래 개선을 묶음별 md(관련 코드 위치·수정 상세·불변 보장·체크리스트·status)로 관리하는 백로그다. `README.md` 가 인덱스.
- 리뷰 중 발견한 **비차단(당장 안 고쳐도 되는) 개선/드리프트**는 보고만 하고 흘리지 말고, **해당 백로그 문서에 추가하거나(맞는 묶음이 있으면) 새 md 로 등재**해 나중에 작업할 수 있게 한다(예: 스키마엔 있으나 렌더러가 안 쓰는 필드, 튜닝 무시 같은 잠재 버그). 차단성 오류만 즉시 라우팅.
