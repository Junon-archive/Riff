...
## 업데이트 백로그 (참고)
- 새 카피가 필요해지는 계획된 기능은 `_design_docs/05_update_backlog/` 에 있다(예: 05=태그·필터 라벨 3언어, 01=커리큘럼 소개 `intro` 3언어). 해당 항목 착수 시 필요한 i18n 키를 그 md 기준으로 작성하고 status 를 확인한다. `README.md` 가 인덱스.
