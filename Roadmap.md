...
| **Functions(연습 도구)** | ✅ 1호 라이브 / 🟡 확장중 | **메트로놈**(`/tools/metronome/`, 3언어) + 공통 셸(랜딩 카드·레슨 FAB→플로팅 오버레이·지연 로드). 음색 = 합성 클릭 + **드럼 킷**(CC0 확보·검증 완료), 목소리는 샘플 대기. 반주(21)·튜너(22)·루프(23)는 "곧 열려요" |
...
- [ ] **M5 Functions(연습 도구)** — 메트로놈·배킹트랙·(튜너·루프스테이션) ⬅ **다음** (설계 확정 중)

---

## 4. 변경 로그 (Changelog)

### 2026-07-24 (백로그 20 후속 3 — **세분 시각화 도입**(확정 결정 B0-r2 변경) + 정지 시 점등 해제)
- **설계 변경(사용자 결정):** B0/B4 의 "세분음 시각 이펙트 없음"을 뒤집어 **세분을 시각화**한다 → 박 동그라미 아래 **점 개수 = 세분 수**(♩1 · 8분2 · 셋잇단3 · 16분4), 재생 중 **지금 울리는 세분 점 하나만** 점등. 숫자 동그라미는 그 박 내내 켜져 있고, 강박 파동은 박 머리(`sub === 0`)에서만. 20_metronome.md 의 B0 표·B4·구현 기록을 **B0-r2** 로 갱신(취소선 + 변경 사유 명시).
- **구현:** 엔진 `BeatEvent` 에 `sub` 인덱스 추가(이전엔 박 머리만 시각 큐에 넣었음) → rAF 가 해당 점을 칠한다. 패널은 `renderBeats(meter, subdivision)` 로 점 줄을 다시 그리고, 세분/박자 변경 시 재렌더. 점은 개수가 늘수록 살짝 작아진다(6px → 5px, 4개일 때 gap 2.5px)라 숫자 원(34px) 폭 안에 들어간다.
- **🐛 함께 수정:** 정지 버튼으로 멈췄을 때 **마지막 점등이 남아 "울리는 중"처럼 보이던 문제** — 하이라이트 해제가 `stopMetronome()`(레슨 이동용)에만 있었다. `onPlayingChange(false)` 한 곳으로 옮겨 정지 경로 전부(버튼·레슨 이동·재바인딩)에서 지워진다.
- **검증:** 헤드리스 계측 10/10 — 세분 1~4 각각 점 개수 일치 · 6/8+16분 = 동그라미 6 × 점 4 · 점 줄 28px ≤ 원 34px(넘침 없음) · 재생 중 항상 **점 하나만** 점등 · 세분 인덱스 0·1·2 순차 등장 · 정지 후 점등 0. `astro check` 0 errors/0 warnings · build 1009 페이지 exit 0.

### 2026-07-24 (백로그 20 후속 2 — 강박 파동 정렬 버그픽스 + 세분 첫 라벨 카피 수정)
- **🐛 강박 파동(ripple)이 숫자 원과 어긋나 보이던 문제:** 실측 결과 좌표 계산은 맞았고, **`::before` 의 `box-sizing` 이 `content-box`** 라 테두리 2px 이 박스 **바깥**에 붙어 실제 그려지는 원이 우하단으로 2px 밀려 있었다. 원인은 `app.css` 전역 리셋 `* { box-sizing: border-box }` 가 **의사요소를 덮지 않는다**는 점(범용 선택자는 `::before/::after` 에 적용 안 됨). 해당 규칙에만 `box-sizing: border-box` 를 명시해 수정(전역 리셋은 다른 컴포넌트 영향 우려로 건드리지 않음). 아울러 매직넘버(`top:-5px`·`margin-left:-22px`)를 `--beat-size`/`--ripple-size` 변수 기반 중앙 정렬 식으로 교체 — 원 크기를 바꿔도 정렬이 따라온다. **검증:** 브라우저 실측 어긋남 X 0.00px · Y 0.00px, `transform-origin` 도 참(painted) 중심으로 정정. 정지 프레임 캡처로 동심원 육안 확인.
- **카피:** 세분 첫 옵션 ko `박만` → **`기본`**(어색한 표현 교체, 사용자 지정). en `Beats only` · ja `拍のみ` 는 유지.
- **검증:** `astro check` 0 errors/0 warnings · build 1009 페이지 exit 0.

### 2026-07-24 (백로그 20 후속 — 드럼 킷 샘플 확보·검증 + 첫 박 폴백 버그 수정)
- **드럼 3종 드롭인:** `web_app/public/metronome/drum/{kick,snare,hihat}.wav`(44+44+22 = **110KB**). 빌드타임 탐지가 그대로 물어 **코드 수정 0으로 「드럼」 음색 자동 활성화**(계약 §B3-1 의 확장자 폴백 `m4a→mp3→ogg→wav` 가 wav 를 집어냄). 목소리는 여전히 "준비 중".
- **파일 실측:** 전부 mono/16bit/44.1kHz · 길이 500·500·250ms · **선행 무음 0.16~0.20ms**(≈8샘플 — 박 밀림 없음) · 피크 −4.79/−2.88/−5.04 dBFS.
- **킷 매핑 실동작 검증(헤드리스 계측 6/6):** 예약된 AudioBuffer 길이 + GainNode 값을 가로채 확인 — 4/4 `킥·스네어·하이햇·스네어`, 6/8 `킥 + 4번째 8분 스네어 + 나머지 하이햇`, 3/4 `킥 + 하이햇 2개`(백비트 없음), 세분음 `하이햇 0.22 게인`. 전부 설계표대로.
- **🐛 수정(검증 중 발견):** 드럼을 골라도 **재생 첫 한두 박이 합성 클릭("틱")으로 새어 나오는** 문제 — 샘플 fetch/decode 를 `start()` 안에서 시작해 첫 박 예약을 못 맞췄다. ① 패널 mount·음색 변경 시 `engine.prefetch()` 가 **AudioContext 없이 바이트만** 선(先)수신(자동재생 정책 경고 없음), ② `start()` 는 재생 상태를 UI 에 즉시 반영한 뒤 버퍼 준비를 기다렸다 스케줄 시작(대기 중 정지하면 취소). 계측상 **첫 박부터 폴백 0**.
- **검증:** `astro check` 0 errors/0 warnings · build 1009 페이지 exit 0 · 기존 e2e **28/28** 재통과 · 드리프트 시뮬 10분 누적오차 0.000ms 유지.
- **남은 것:** CC0 **목소리(One~Six) 6개**만. 선택 최적화로 드럼 wav 트림+`.m4a` 인코딩(110KB→~10KB) 여지 있음(현재도 소형이라 비차단).
...
### 2026-07-24 (배킹 21 — 프롬프트 뱅크 3세트 완성: 드럼비트 11 + 기타 36 + 베이스 36 = 83트랙/27파일)
- **범위:** 21_backing ★R3 3세트 매트릭스 갱신에 맞춰 프롬프트 뱅크를 **드럼비트·기타·베이스 3세트로 재편·완성**. 문서만 — 음원 생성·구현 미착수.
- **드럼비트 세트 신설(`backing_prompts/drums/`):** 조 없는 순수 드럼 키트, **기타·베이스 공유**. 3필 × 필별 템포 = **11트랙** — `straight.md`(락 백비트, 70·95·120·150·200 · `{FEEL}` 템포대별 치환) · `funk.md`(16분 고스트, 90·100·110) · `shuffle.md`(트리플렛, 70·90·120). 전부 16마디(8·16마디째 필). 파일명 `{feel}_{bpm}.m4a`. **구 `guitar/drums.md` 는 이 세트로 이관·삭제**(드럼이 기타 세트 안에 있던 구조 해소).
- **베이스 세트 저작(`backing_prompts/bass/` 12파일 36트랙):** 기타와 장르·조·템포·코드 진행 **완전 미러**, 차이는 **베이스 전면 제외**(드럼+코드 컴핑만 남고 사용자가 베이스를 넣음)뿐. 베이스 핵심조 **E·Em·F** 전부 포함(shuffle·입문베이스=E, slap=Em, walking=F). `bass/README.md` = 서브 인덱스 + **우선순위 Tier 1~3**(각 12트랙, Tier 1 = 커리큘럼 직결) + **★ 베이스 세트 공통 함정**.
- **베이스 고유 함정 문서화(기타 세트엔 없던 실패 유형):** ① Exclude `bass` 탓에 **킥까지 얇아짐** → 킥 살아있는 테이크 선별 ② **다른 악기가 베이스를 대신함** — 재즈 피아노 왼손 워킹·네오소울 로즈 왼손 루트·블루스 기타 저음현 부기·rock_E 저음현 파워코드 → 프롬프트에 `right hand only`·`no left-hand bass notes`·`middle register` 삽입 ③ 저역이 비는 게 정상 ④ 트림 기준점이 베이스가 아니라 드럼/컴핑.
- **용량 재산정 ≈ 49 MB**(드럼 6.7 + 기타 21.0 + 베이스 21.0 / ≈50.6분). R3 추정 ~42MB 대비 상향 — 비블루스 폼을 8마디가 아닌 **16마디**로 확정했기 때문(R1 범위 내, 재즈 160BPM에서 8마디=12초라 반복 티가 심함). 예산 80MB의 61%, 부족 시 8마디로 ~10MB 회수 가능.
- **연동 갱신:** `backing_prompts/README.md` 인덱스 3세트 재구성(총계 표·파일명 규약에 `{feel}_{bpm}` 추가·저장 경로 `drums/` 추가·워크스루의 드럼 예시와 테이크 검수 표에 베이스/드럼 항목 추가·체크리스트 3세트화) · `21_backing.md` 프롬프트 뱅크 섹션·체크리스트·헤더 갱신 · `05_update_backlog/README.md` 21행.
...
- **확정 방향(사람):** 랜딩 상단 도구 카드 4개(튜너·메트로놈·반주·루프, 각 전용 페이지) · 레슨 내 **우하단 FAB→플로팅 오버레이**(Segmented 메트로놈/배킹, 커리큘럼 이탈감 X, 재생 중 FAB 아이콘 변화) · **레슨 이동 시 정지**(persist 불필요) · 오디오 코드 **지연 로드**로 정적 구성 유지 · 기타 파랑/베이스 초록 테마.
- **메트로놈:** Web Audio 룩어헤드 스케줄러 · 합성 클릭(자산 0) + 드럼/목소리 샘플 슬롯(CC0, 파일 후속) · 2/4·3/4·4/4·6/8 · 세분 · 탭템포 · 30~300 BPM · 박 글로우+강박 파동.
- **배킹:** 정적 음원(Suno Pro, 재배포 라이선스 확인) · 템포=별도파일(피치보존) · 심리스 루프(Web Audio) · 매니페스트 주도 캐스케이딩 버튼 · **커리큘럼 조 검증 반영**(모든 장르 Am 공통 + 각 장르 연계 커리큘럼 조: 블루스 A·펑크 E/Em·재즈 C/F·네오소울 Dm 등) · 확정 매트릭스 기타 41 + 베이스 36 ≈ 77트랙/~39MB(예산 ≤80MB, CF Free 20k파일·25MiB/파일 여유).
- **상태:** **20·21 설계 확정 완료(구현 대기).** 메트로놈 결정 마감(목소리 180BPM↑ 비활성·킥/스네어/하이햇 키트·6/8 6점·세분 소리만·이퀄라이저 FAB). 배킹 매트릭스 확정(락/팝 G 유지·블루스 A=Am겸용·드럼만 16마디 8/16변주·장르확장 후보만 기록). README 인덱스에 Functions 섹션 추가. **구현·커밋 미착수.**
