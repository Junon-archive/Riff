...
│   ├── render/
│   │   ├── index.ts              renderScore 디스패치 (type/notation → 렌더러 선택)
│   │   ├── fretboard.ts          지판 SVG 렌더 (schema.fretboard → SVG)
│   │   └── staff.ts              오선보+타브 결합 렌더 (VexFlow, 모든 tab 블록)
...
### 5.3 `render/tab.ts` — **제거됨(백로그 18, 2026-07-11)**

구 자체 타브 SVG 렌더러(`renderTab`)는 **삭제**되었다. 모든 `type=tab` 블록은 이제
VexFlow 오선보+타브 결합 렌더러(§5.4 `render/staff.ts`)로 그린다.

- 배경: `staff.ts` 도입 후 solo_scale_3months 89블록만 `meta.notation` 미지정이라
  구 tab.ts 로 라우팅되고 있었다. 백로그 18 에서 이들에 `"notation":"staff+tab"` 를 부여해
  전 커리큘럼(빌드되는 type:tab 블록 전부)이 staff+tab 로 통일 → tab.ts 사용처가 0 이 되어 제거.
- `renderScore` 는 `meta.notation` 미지정 시 **`staff+tab` 로 폴백**한다(§5.5).
- H/P/sl/bend/vib/harmonic 등 표현 기법 오버레이 계약은 §5.4 staff.ts 로 승계되었다.
- `.tabsvg` 컨테이너 스타일 계약(`design_spec.md` §4.11)은 유지(staff.ts 는 `.staffsvg` 사용).
...
- **모든 `type=tab` 블록**에 사용(§5.5 `renderScore` 가 라우팅). `meta.notation` 이 `"staff"|"rhythm"` 이면 해당 모드, `"staff+tab"`/미지정이면 오선보+타브 결합. fretboard 만 자체 SVG(§5.2) 유지(구 tab.ts 는 §5.3 대로 제거).
...
- **technique 렌더(오선보/타브 오버레이 — v1 한계였던 부분, 이제 지원):**
  ...
  - **데이터 저작 지침:** 펑크의 뮤트 스크래치·고스트는 `dead_note`, 진짜 팜뮤트만 `palm_mute`(구분: `color_legend.md`/`00_curriculum_authoring_playbook.md`·`fretboard_score_schema.json`). H/P/sl/bend/vib/harmonic 등 표현 기법 오버레이도 이 staff.ts 가 담당(구 tab.ts 계약 승계).

### 5.5 디스패치 & 마운트 계약

- `render/index.ts` 의 **`renderScore(score): string`** 가 단일 진입점: `type=fretboard_diagram|scale_shape`→`renderFretboard`, `type=tab`→`renderStaff`(`meta.notation` `"staff"|"rhythm"`→해당 모드, `"staff+tab"`/미지정→오선보+타브 결합 폴백).
...
- **악보 SSR:** `render/fretboard.ts`·`render/staff.ts` 가 **SVG 문자열** 을 반환(순수)하므로 `set:html` 로 서버 렌더 → 지판/오선보+타브가 색인 가능한 HTML 로 출력(Zero-Hallucination 유지, 클라이언트 JS 불필요).
...
- **산출물:** `src/render/fretboard.ts`, `src/render/staff.ts`, `src/render/index.ts`, `src/types/score.ts`.
