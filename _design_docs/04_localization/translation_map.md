...
## 10. 랜딩 악기 세그먼트 토글 — `home.title` 악기별 분화 (2026-07-10, 05_tag-filter 단계 2)

기타/베이스 세그먼트 토글 도입에 맞춰, 기존 단일 문자열이던 `home.title`을 **중첩 객체 `home.title.guitar` / `home.title.bass`** 두 변형으로 분화했다(3언어 동시). 베이스 변형은 기타 변형에서 **악기를 가리키는 명사만 치환**하고 나머지 문장·HTML(`<br>`, `<span class='accent'>…</span>`)은 완전 동일하게 유지한다(별도 작문 없음).

| key | ko | en | ja |
|---|---|---|---|
| `home.title.guitar` (구 `home.title`, 값 불변) | 기타가 어려웠던 이유,<br>\<span class='accent'\>연습 순서\</span\>가 없어서였어요 | Guitar felt hard because<br>\<span class='accent'\>the path was unclear\</span\> | ギターが難しかった理由は、<br>\<span class='accent'\>練習の順番\</span\>がなかったから |
| `home.title.bass` (신규 — 악기 명사만 치환) | 베이스가 어려웠던 이유,<br>\<span class='accent'\>연습 순서\</span\>가 없어서였어요 | Bass felt hard because<br>\<span class='accent'\>the path was unclear\</span\> | ベースが難しかった理由は、<br>\<span class='accent'\>練習の順番\</span\>がなかったから |

- `instrument.guitar`/`instrument.bass`/`filter.all`은 §8(2026-07-03)에서 이미 3언어 신설 완료 — 이번 작업에서 재확인만 하고 변경 없음.
- `home.title`이 leaf 문자열 → 객체로 바뀌었으므로, **`translate(lang, 'home.title')`을 leaf로 직접 호출하는 코드는 반드시 `home.title.guitar`/`home.title.bass`로 갱신해야 한다**(dot-path resolver는 객체를 문자열로 취급하지 못해 키 원문을 그대로 반환함). 발견된 참조 3곳(프론트 담당 수정 대상):
  - `web_app/src/pages/[lang]/index.astro:17`
  - `web_app/src/pages/index.astro:11`
  - `web_app/src/components/HomeView.astro:32` (`set:html`)
- `home.title` 분화로 leaf 키 순증 +1(기존 1개 → `guitar`/`bass` 2개). 현재 `web_app/src/i18n/{ko,en,ja}.json` 언어당 **총 121개 leaf 키**, 3언어 키셋 diff **0건**(재귀 leaf-path 비교로 검증).
