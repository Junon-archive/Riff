...
8. **`notation:"rhythm"`과 슬래시 노트헤드는 현재 렌더러 미지원 → 사용 금지.** 컴핑·리듬 자료도 `"staff+tab"`으로 낸다(음정 노트헤드 + 박자 공유). `meta.notation` 유효값은 `"tab"`|`"staff"`|`"staff+tab"`이며, `"rhythm"`은 enum에 남아 있으나 실제로는 오선보(음정 노트헤드)로 폴백 렌더되므로 저작 시 지정하지 않는다.
