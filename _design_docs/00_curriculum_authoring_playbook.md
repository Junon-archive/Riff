...
### 악보 데이터 표기 규약 (렌더 계약 — 8규칙)

현행 렌더러(`web_app/src/render/*` · 타입 `src/types/score.ts`)가 실제로 지원하는 표기법. 신규·기존 커리큘럼의 **생성 프롬프트와 콘텐츠는 이 8규칙을 따른다.** (스키마 SSOT: `03_data_schema/fretboard_score_schema.json`.)

1. **컴핑(코드 반주)은 `chord[]`(동시타)로 낸다.** 대표음(`TabNote.string`/`fret`)은 그 화음의 **최저음**(string 번호가 가장 큰 음), 나머지 음은 `chord[]` 배열에 담는다. 기존 리듬 타점에 스택만 얹는 방식이라 리듬이 보존된다 — **`chord[]`는 마디 박자합에 기여하지 않는다**(박자는 대표음의 `duration`으로만 계산). 구 "코드는 대표음 하나로 리듬만 표기" 방식을 대체한다.
2. **화음 각 음의 `role`·`label`은 같은 day 지판(fretboard) dot과 일치**시킨다. 화음에서는 **도수 `label` 텍스트가 렌더되지 않으므로**, 도수 정보는 **색(`role`)** 으로 전달된다 — root/chord_tone=**파랑**, target/highlight=**초록**, color=**노랑**(색 SSOT: `color_legend.md`).
3. **단선율 라인은 단음으로 둔다.** 가이드톤 라인·탑노트 멜로디·싱글노트 리프는 `chord[]`로 합치지 말고 한 음씩 낸다.
4. **셔플/스윙은 `meta.feel`로 지정**한다(`"swing8"` | `"swing16"`). 악보 음표는 정박으로 두고 렌더러가 오선보 위에 "Swing 8ths/16ths"를 표기한다. 산문·제목으로만 "스윙"이라 적고 `feel`을 비우는 방식은 쓰지 않는다(`feel`은 유효 필드다 — "feel 필드 없음/금지" 류 서술 금지).
5. **악센트는 `role:"target"` + `highlight`(초록)** 로 표현한다. `">"` 악센트 기호나 업/다운 피킹 기호는 렌더러가 그리지 않으므로 쓰지 않고, 세기·피킹 방향의 뉘앙스는 산문으로 설명한다.
6. **고스트/뮤트 커팅 = `technique:"dead_note"`**(음정 없는 뮤트 타격 → 타브·오선보에 X). **`palm_mute`는 음정이 있는 실제 팜뮤트(P.M.)** 에만. 둘은 별개다(§3 상단 "데드 노트 vs 팜뮤트" 참조).
7. **`stroke`는 아르페지오(펼침 `"arpeggio"`)에만.** 동시 스트로크(스트럼)는 `stroke`를 지정하지 않는다 — 화음(`chord[]`) 표기 자체가 동시타를 뜻한다.
8. **`notation:"rhythm"`(슬래시 리듬 악보) 지원됨(2026-07-09, 백로그 04).** 코드 반주를 음정 대신 **빗금(슬래시) + `chordSymbol`**으로 표기하는 리듬 차트. staff.ts가 노트헤드를 숨기고 가운데 줄에 자체 슬래시 바(path)를 그린다. 유효값 = `"tab"`|`"staff"`|`"staff+tab"`|`"rhythm"`. 리듬 표기 위주 자료는 `"rhythm"`(음정 불필요, `chordSymbol`로 코드 지정), 음정이 중요한 컴핑은 `"staff+tab"`(실음 노트헤드 + 박자 공유)로 낸다.
