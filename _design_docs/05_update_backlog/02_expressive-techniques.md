...
>
> **🐛 후속 버그픽스 ③ (2026-07-11) — 잇단음 내 쉼표 빔 오연결:** "가운데 빼기" 셔플(`[음, 8분쉼표, 음]` 트리플렛)에서 **빔이 박 경계를 넘어 잘못 연결**(앞 박 3번째 음 ↔ 뒷 박 1번째 음). **원인:** rest 처리 경로가 `tuplet` 을 무시하고 `flush()/flushTup()` 을 호출 → 쉼표가 빔·튜플렛 그룹을 끊고 이후 실음들이 박 경계를 넘어 재그룹됨. **수정:** rest 도 `tuplet` 이면 튜플렛 그룹에 **포함**(브래킷·tick 정합)하되 **빔엔 미포함**(빔은 앞뒤 실음끼리 쉼표 위로 span) — 완료 판정·pos 전진은 실음 브랜치와 동일. **재현:** `shuffle_bounce_bass_2months/m1/w1/d2` "Drop the middle". **검증:** build 0 · check-invariants 내 변경 회귀 0(별도 solo_scale 57블록 변경은 **동시 세션 백로그 18** 이식분) · 셔플 빔 마디당 4(박당 1빔, 쉼표 위 span). 육안은 배포 후.
