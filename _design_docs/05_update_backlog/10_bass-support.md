...
> ℹ️ **예정 — 베이스 UI 액센트=연초록(2026-07-10):** 이 문서는 베이스 **악보/엔진**만 다룬다.
> 베이스 커리큘럼/레슨의 **UI 테마 액센트를 연초록**으로 주는 작업은 **백로그 05**(악기별 액센트 테마) 소관 —
> `[data-instrument="bass"]` 토큰 오버라이드, 콘텐츠 기반(`meta.instrument`) 적용. 여기서 구현하지 않는다.
