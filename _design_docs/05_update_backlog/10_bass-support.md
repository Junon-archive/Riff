...
4. **베이스 튜닝 기본값(4/5/6현)** — 4현 EADG, 5현 BEADG, 6현 BEADGC. 03은 "정의 위치 미정"으로 남겨둠.
5. **베이스 지판(fretboard.ts)** — `N_STR=6` 하드코딩 + **프로젝트 규칙상 변경 금지 대상**(별도 승인 필요).
...
| **03 ③** | `stringCount` 리터럴 6 → 허용 집합, 타브 줄 수·string 범위 stringCount 구동 | 4현(5·6현) 타브·검증 | **하드(필수)** |
...
> **결론:** 03 ②③ 완료가 이 문서 Phase B1~B3의 전제. 03을 먼저 끝내거나, 최소 03②③를 이 작업의 Phase 0으로 흡수해 함께 진행한다(아래 Phase B0).
...
| **지판 6현 하드코딩** | `fretboard.ts:21` `N_STR=6` + string 1~6 검사 | 6현 지판만 | ⚠️ **변경 금지 대상** — 베이스 지판 필요 시 별도 승인 |
...
03의 ②(튜닝 계산)·③(다현 일반화)를 **먼저 완료**하거나 이 작업에 흡수한다. 완료 기준은 03 문서의 체크리스트·V4 골든 스냅샷(기존 6현 픽셀 불변). **여기서 stringCount 허용 집합에 `5`를 포함**하도록 03③를 확장(4·5·6). 이후 B1~B4는 이 위에 얹는다.
...
5. **기존 불변:** `instrument` 미지정/`guitar` 는 전 분기 기존과 동일 → 6현 커리큘럼 오선보 **바이트 불변**(V4 골든 스냅샷).
...
### Phase B3 — 베이스 튜닝 기본값 + stringCount 5 (다현 완성)

1. **기본 튜닝 정의(SSOT 위치 = 렌더 계산 함수 + 스키마 description):** 4현 `["E","A","D","G"]`(저→고, index0=최저), 5현 `["B","E","A","D","G"]`, 6현 `["B","E","A","D","G","C"]`. **주의:** tuning 배열 순서 규약은 기존과 동일하게 **6→1(최저→최고 인덱스)**, 현 번호는 1=최고음. 4현이면 string 1~4.
2. **stringCount 집합에 5 포함**(B0에서 4·6과 함께 4·5·6). 스키마 enum·타입·검증기 `build-content.mjs:165` 3중 갱신.
3. **string 범위 검사**를 stringCount 구동(4현이면 1~4, dot·note·chord·barre 전부).
4. **검증:** 4현 EADG 픽스처로 타브 4줄·bass clef 음정 정확 확인, 5현 픽스처로 저음 B 확장 확인.

### Phase B4 — 베이스 지판(fretboard.ts) — ⚠️ 변경 금지 대상, 별도 승인 필수

- `fretboard.ts` 는 프로젝트 규칙상 **변경 제안 금지**(03 §주의에도 명시). 베이스 지판 다이어그램(4/5/6현, 현 이름=베이스 튜닝, 저음현 굵기)이 필요하면 **사용자 승인 후** 착수.
- **회피 옵션:** 초기 베이스 커리큘럼을 **`tab`·`scale_shape`·`staff+tab` 중심**으로 저작하고 `fretboard_diagram`(코드 지판 다이어그램)은 최소화하면 B4를 미루고도 트랙 시작 가능. (베이스는 코드 다이어그램 수요가 기타보다 낮음 — 라인/스케일/리프 중심.)
- 승인 시: `N_STR` 을 stringCount 구동으로, 현 이름을 tuning에서, 좌표·굵기 일반화. V4 골든 스냅샷으로 6현 지판 픽셀 불변 필수.
...
- **스키마 게이트:** B1~B3(필요 시 B4) 렌더 확장이 **먼저 머지된 뒤** day 저작(안 그러면 베이스 악보가 안 그려지거나 틀리게 그려짐). 즉 콘텐츠는 엔진에 의존.
- **커리큘럼 설계 요소(교육):** 오른손(핑거·투핑거·레이킹)·팜뮤트·고스트노트·슬랩(썸/팝)·해머/풀·슬라이드·펜타/메이저 스케일 라인·루트-5도-옥타브·워킹베이스 기초. 난이도 곡선·estMinutes 등은 `00_curriculum_authoring_playbook` 준수.
...
- **베이스 신규 픽스처:** 4현 EADG로 (1) 타브 4줄 (2) bass clef 음정 정확 (3) slap T/P 표기 (4) 5현 저음 B 확장 육안·스냅샷.
- **V5** solo/chord/funk 등 미대상 커리큘럼 산출물 바이트 불변.

## 주의 / 정지조건

- **03②③ 없이 착수 금지:** 튜닝 계산·다현 일반화가 선행. 없이 B1~B3 하면 음정이 틀리거나 타브 줄 수가 안 맞음. **순서 엄수.**
- **`fretboard.ts` 변경 금지(B4):** 별도 승인 전 손대지 않는다. 초기 트랙은 지판 다이어그램 최소화로 우회.
...
- **콘텐츠는 엔진 뒤에:** day 저작(B5)은 B1~B3(필요 시 B4) 머지 후. 엔진 미비 상태로 베이스 악보 저작하면 렌더 실패.
- **T 표기 충돌(tap vs slap-thumb):** 두손 태핑을 도입하면 `T`(썸)와 표기 충돌 → 태핑은 별 글리프/괄호로. 미해결 질문 참조.

## 체크리스트

**Phase B0 — 선행(03②③)**
- [ ] 03② 튜닝 계산(OPEN_MIDI 대체) 완료 + 골든 스냅샷 6현 불변
- [ ] 03③ stringCount 집합화(4·5·6) + 타브 줄 수·string 범위 stringCount 구동
...
**Phase B3 — 베이스 튜닝·다현 완성**
- [ ] 기본 튜닝 정의(4 EADG / 5 BEADG / 6 BEADGC) + 순서 규약 문서화
- [ ] stringCount 5 포함, string 범위 stringCount 구동
- [ ] 4현·5현 픽스처 타브/음정 검증

**Phase B4 — 베이스 지판(승인 필요)**
- [ ] (사용자 승인) fretboard.ts 다현화 or 초기 트랙 지판 최소화로 보류
- [ ] 승인 시 골든 스냅샷 6현 지판 불변

**Phase B5 — 베이스 커리큘럼 저작**
- [ ] 엔진(B1~B3, 필요시 B4) 머지 확인 후 착수
...
- **베이스 현 수 범위:** 출시 시 4현만 vs 4·5·6현 전부 스키마 지원(콘텐츠는 4현부터). — 권장: **스키마는 4·5·6 준비, 콘텐츠는 4현 트랙 우선.**
- **베이스 지판(fretboard.ts) 다현화:** 지금 승인해 B4 포함 vs 초기 트랙 지판 최소화로 보류. — 권장: **보류(라인/스케일/리프 중심 저작)**, 수요 확인 후 승인.
...
- **estMinutes·난이도:** 왕초보 베이스(30분?) vs 중급(펑크 슬랩) — 어느 페르소나 우선.
