...
> **3세트 = 드럼비트 11 + 기타 36 + 베이스 36 = 83트랙** (21_backing ★R3).

### ① 드럼비트 세트 (`drums/`) — 3 파일 / 11 트랙 · **기타·베이스 공유**

조 없음 · 순수 드럼 키트(멜로디·코드 0) · 16마디(8·16마디째 필). 파일명 `{feel}_{bpm}.m4a`.

| 파일 | 필(feel) | 폼 | 템포(BPM) | 트랙 | 성격 · 연계 |
|---|---|---|---|---|---|
| [`drums/straight.md`](drums/straight.md) | 스트레이트/락 백비트 | 16마디 | 70·95·120·150·200 | 5 | 범용 백비트 — 전 장르 기본 타이밍 |
| [`drums/funk.md`](drums/funk.md) | 펑크 16비트 | 16마디 | 90·100·110 | 3 | 16분 고스트 그루브 (funk·slap) |
| [`drums/shuffle.md`](drums/shuffle.md) | 셔플/블루스 스윙 | 16마디 | 70·90·120 | 3 | 트리플렛 셔플 (blues·shuffle) |

**11트랙 · ≈ 7.0분 · ≈ 6.7 MB.** 배치 `web_app/public/audio/backing/drums/`

### ② 기타 세트 (`guitar/`) — 12 파일 / 36 트랙

**리드 기타 제외** (드럼 + 베이스 + 코드 컴핑이 남는다). 파일명 `{장르}_{조}_{bpm}.m4a`.
...
**36트랙 · ≈ 21.8분 · ≈ 21 MB.** 배치 `web_app/public/audio/backing/guitar/`

### ③ 베이스 세트 (`bass/`) — 12 파일 / 36 트랙

**베이스 제외** (드럼 + 코드 컴핑이 남고 사용자가 베이스를 넣는다). 기타 세트와 **장르·조·템포·코드 진행 전부 동일한 미러**.

📌 **[`bass/README.md`](bass/README.md) 가 베이스 세트의 인덱스**다 — 전체 표 · **우선순위 Tier 1~3**(급하면 12트랙만 먼저) · **★ 베이스 세트 공통 함정**(킥 얇아짐 · 피아노 왼손이 워킹을 대신 걸음 등)을 담고 있다. 베이스 트랙을 만들기 전에 반드시 읽는다.

| 장르 | 조 | 템포(BPM) | 트랙 | 커리큘럼 연계 |
|---|---|---|---|---|
| 블루스(셔플) | A · **E** | 70·90·120 | 6 | shuffle베이스(E)·입문베이스(E) |
| 락/팝 | Am · **E** · G | 80·110·140 | 9 | 입문베이스(E)·solo(Am) |
| 펑크 | Am · **Em** | 90·100·110 | 6 | slap베이스(Em) |
| R&B/네오소울 | Am · Dm | 70·85·100 | 6 | neosoul 그루브(Dm) |
| 재즈 ii-V-I | Am · C · **F** | 100·130·160 | 9 | walking베이스(F) |

**36트랙 · ≈ 21.8분 · ≈ 21 MB.** 배치 `web_app/public/audio/backing/bass/` — 파일명은 기타와 동일하고 **디렉터리로 구분**(매니페스트는 `instrument` 필드로 구분). 베이스 핵심조 **E·Em·F** 전부 포함(굵게).

### 총계

| 세트 | 파일 | 트랙 | 길이 | 용량 |
|---|---|---|---|---|
| 드럼비트 | 3 | 11 | ≈ 7.0분 | ≈ 6.7 MB |
| 기타 | 12 | 36 | ≈ 21.8분 | ≈ 21.0 MB |
| 베이스 | 12 | 36 | ≈ 21.8분 | ≈ 21.0 MB |
| **합계** | **27** | **83** | **≈ 50.6분** | **≈ 49 MB** |

> **용량 메모:** 21_backing R3의 추정치는 ~42 MB였는데, **비블루스 폼을 8마디가 아닌 16마디로 확정**하면서 ≈49 MB가 됐다(R1의 "8~16마디" 허용 범위 안). 8마디면 재즈 160 BPM에서 루프가 12초라 반복 티가 심해서 내린 판단이다. **예산 80 MB의 61%** 로 여전히 여유가 있고, 부족해지면 비블루스 폼을 8마디로 줄여 ~10 MB를 회수할 수 있다.

---

## 파일 규약

- **md 1개 = (세트 × 장르 × 조) 1개.** 드럼비트는 조가 없으므로 **md 1개 = 필(feel) 1개**. 같은 조합의 템포들은 프롬프트가 동일하고 BPM만 다르므로 한 파일에 "렌더 타깃"으로 묶는다.
- **파일명 = 오디오 파일명 접두사와 일치.** `guitar/blues_A.md` → `blues_A_70.m4a` · `blues_A_90.m4a` · `blues_A_120.m4a`.
- 오디오 파일명 규약:
  - **키드 트랙**(기타·베이스): `{장르슬러그}_{조}_{bpm}.m4a` — 슬러그 `blues` · `rock` · `funk` · `neosoul` · `jazz`
  - **드럼비트**: 조 없음 → `{feel}_{bpm}.m4a` — feel `straight` · `funk` · `shuffle`
- **기타·베이스는 파일명이 같다.** 디렉터리(`guitar/` · `bass/`)로 구분하고, 매니페스트에서는 `instrument` 필드로 구분한다.
...
2. **제외 파트 명시** — 기타 세트 = 드럼 + 베이스 + 코드 컴핑만, **리드 기타/기타 솔로 제외**(사용자가 그 위에 솔로). 베이스 세트 = **베이스 전면 제외**(드럼 + 코드 컴핑만 남김). 드럼비트 = **드럼 단독**.
...
파일명은 각 md의 「렌더 타깃」 표에 이미 확정돼 있다. 그대로 쓴다 (`blues_A_90.m4a`, `straight_120.m4a` …).
...
```
web_app/public/audio/backing/drums/straight_120.m4a     ← 드럼비트(기타·베이스 공유)
web_app/public/audio/backing/drums/shuffle_90.m4a
web_app/public/audio/backing/guitar/blues_A_90.m4a      ← 기타 세트
web_app/public/audio/backing/bass/blues_A_90.m4a        ← 베이스 세트(파일명 동일·디렉터리로 구분)
```

- 브라우저에서 접근하는 URL: `/audio/backing/guitar/blues_A_90.m4a` · `/audio/backing/drums/straight_120.m4a`
...
83트랙 WAV는 수 GB에 이른다. `.gitignore` 로 막는 것보다 **아예 저장소 바깥 폴더**에 두는 편이 안전하다 — GitHub 저장소 비대화가 이 기능의 실질 제약이고(21_backing 「플랫폼 사실」), 실수로 한 번 커밋되면 히스토리에 영구히 남아 되돌리기 어렵다.
...
> **트랙 1개를 끝까지 완주하는 절차**다. 83트랙을 이 순서로 반복한다.
> 예시는 `guitar/blues_A.md` 의 90 BPM 트랙(`blues_A_90.m4a`)으로 든다.
> 처음이라면 **`drums/straight.md` 의 `straight_120` 한 개를 먼저 끝까지 해보길** 권한다. 드럼은 어택이 선명해 트림 지점을 찾기 가장 쉽고, 여기서 감을 잡으면 나머지가 훨씬 수월하다.
>
> **권장 순서: 드럼비트 11 → 기타 36 → 베이스 36.** 드럼비트는 양쪽이 공유하므로 먼저 확보해 두면 나머지 작업 중에도 바로 쓸 수 있다.
> **베이스 세트를 시작하기 전에 [`bass/README.md`](bass/README.md) 의 「★ 베이스 세트 공통 함정」을 반드시 읽는다.** 기타 세트에는 없던 실패 유형이 따로 있다.
...
1. 위 「인덱스」 표에서 md 파일 하나를 연다. → 예: `guitar/blues_A.md` (베이스는 [`bass/README.md`](bass/README.md) 의 Tier 표에서 고르면 순서가 잡힌다)
...
2. 붙여넣은 텍스트 안의 **`{BPM}` 을 STEP 1의 값으로 바꾼다.**
   ...
   - **`drums/straight.md` 는 `{FEEL}` 도 함께 치환**한다(렌더 타깃 표의 「느낌 힌트」 열 문장을 그대로 붙여넣는다). 다른 파일에는 `{FEEL}` 이 없다.
...
| `drums/*` | 16마디째가 크래시 심벌로 끝난다 · 전자 드럼으로 나온다 | 루프 끝에서 "쨍" 하고 잘리면 다른 테이크 |
| `drums/shuffle` | 셔플이 도중에 스트레이트로 풀린다(특히 120) | 트리플렛 느낌이 끝까지 유지되는지 |
| `bass/jazz_*` | **피아노 왼손이 워킹 라인을 대신 걷는다** | 저역에 음정 있는 소리가 남았으면 폐기 |
| `bass/neosoul_*` | **로즈 왼손이 저음 루트를 잡는다** | 위와 동일 |
| `bass/*` 전체 | Exclude `bass` 탓에 **킥까지 얇아진다** | 킥이 살아 있는지 |
...
7. **잔향 처리** — 끝에서 심벌·로즈 서스테인이 뚝 끊기면, 끝부분에 **10~30ms 짧은 크로스페이드**를 준다(루프 시작 구간을 겹쳐 넣는 방식). `neosoul_*` · `jazz_*`(라이드) · `drums/*` 에서 특히 필요하다.
...
## 83트랙을 다 끝낸 뒤 (구현 세션이 이어받음)

1. 매니페스트 `backing.json` 작성 — 트랙마다 `{ id, instrument:"drums"|"guitar"|"bass", genre(드럼비트는 feel), key|null, scale, bpm, file, loopStart, loopEnd, credits, license:"Suno-Pro" }`
2. 총 용량 실측 → 목표 **≈49 MB**, 상한 80 MB (21_backing **V5**)
...
- [x] **드럼비트 3개 md** (straight / funk / shuffle) — 11트랙
- [x] **기타 12개 md** (blues×2 / rock×3 / funk×2 / neosoul×2 / jazz×3) — 36트랙
- [x] **베이스 12개 md** (동 구성 미러) — 36트랙 · [`bass/README.md`](bass/README.md) 서브 인덱스 포함

## 음원 생성 (사람 · Suno Pro) — 「사람이 순서대로 따라 하는 방법」대로 진행

**① 드럼비트 11트랙** — 양쪽 공유이므로 먼저 확보
- [ ] `straight` 5 (70·95·120·150·200)
- [ ] `funk` 3 (90·100·110)
- [ ] `shuffle` 3 (70·90·120)
...
**③ 베이스 36트랙** — 시작 전 [`bass/README.md`](bass/README.md) 「★ 공통 함정」 읽기. Tier 순으로 진행.
- [ ] Tier 1 (커리큘럼 직결 E·Em·F) — `blues_E` 3 · `rock_E` 3 · `funk_Em` 3 · `jazz_F` 3
- [ ] Tier 2 (공통 Am) — `rock_Am` 3 · `funk_Am` 3 · `neosoul_Am` 3 · `jazz_Am` 3
- [ ] Tier 3 (확장) — `blues_A` 3 · `rock_G` 3 · `neosoul_Dm` 3 · `jazz_C` 3

## 후처리
- [ ] 83트랙 루프 트림 + 심리스 검수
- [ ] .m4a 128k 인코딩 + 용량 실측(목표 ≈49 MB, 상한 80 MB)
