...
1. `stringCount` 를 리터럴 6에서 **허용 집합(예: 4·6)** 으로 완화 → 타입·스키마·검증기 3곳.
...
- **V4 골든 스냅샷(필수):** ② 튜닝 리팩터 전/후 전 커리큘럼 오선보 SVG 바이트 동일.
- 신규: Drop-D 픽스처로 음정이 반음 내려가 그려지는지, 4현 픽스처로 타브 4줄·음정 정확 확인.

## 주의 / 정지조건

- ★`render/fretboard.ts` 는 프로젝트 규칙상 변경 제안 금지 대상 — 다현이 지판 렌더까지 필요하면 별도 승인 후 진행.
