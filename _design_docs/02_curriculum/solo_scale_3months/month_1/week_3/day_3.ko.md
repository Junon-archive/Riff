...
그리고 잊지 마세요. 이 런의 목적은 손가락 자랑이 아니에요. 넥 어디를 지나든 **C(♭3) 타겟이 어디쯤 있는지**를 몸에 각인하는 거예요. 런 중간중간 빨간 C를 짚을 때마다 "여기 C, 저기도 C" 하고 위치를 눈에 담으세요. 내일 즉흥에서 이 지도가 무기가 돼요.

## ② 시각 자료

먼저 오늘의 지도, **넥 전체 A 마이너 펜타토닉**이에요(넛~15프렛). 5개 블록이 사실은 끊김 없이 이어진 하나의 그물망이라는 걸 보여줘요. 빨간 강조가 전부 C(♭3) 타겟이에요 — 넥 전체에 C가 이렇게 골고루 흩어져 있어요. (5번 블록 꼭대기의 16·17프렛 음은 5프렛 부근 패턴이 한 옥타브 반복된 것이라 이 지도의 아래쪽에서 이미 같은 모양으로 확인돼요.)
