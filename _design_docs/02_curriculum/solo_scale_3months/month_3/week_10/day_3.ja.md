...
まずは**Aミクソリディアン Position 2**です。根音Aは4弦7フレット。ここでも長3度C#（赤）と♭7 G（赤）の位置を目に馴染ませてください。Position 1と音名は同じで、形だけが違います。
...
2つ目は、2つのフォームをつなぐ**接続マップ**です。Position 1とPosition 2の根音（A）、長3度C#（赤）、♭7 G（赤）をネック上にまとめて並べました。4弦7フレットの根音は、2つのフォームが共有する「つなぎ目」です。そして1弦5フレット（A）→9フレット（C#）のスライドが、今日の移動ルートです。
