...
오늘 왜 이걸 하냐면요. 어제 A와 C 사이에 B라는 다리를 놨죠. 오늘은 마지막 다리 하나를 더 놓으면 A 내추럴 마이너가 완성돼요. 바로 **F(6도)** 예요. F는 펜타토닉의 5도(E)와 ♭7(G) 사이, 정확히는 **E 바로 반음 위**에 붙는 음이에요. 그리고 이 "반음 위"라는 게 오늘의 핵심이에요.

반음 차이는 기타에서 가장 **끈끈한 자석** 같은 관계예요. F는 바로 아래 E로 당겨지고 싶어서 안달이 난 음이거든요. 그래서 F의 가장 자연스러운 해결은 **F → E (6도 → 5도)**, 반음 미끄러지듯 내려오는 거예요. 이게 얼마나 서정적으로 들리는지 오늘 직접 느낄 거예요. B가 위로(C로) 올라가는 다리였다면, F는 아래로(E로) 내려오는 다리인 셈이죠. 방향이 반대라 둘을 같이 쓰면 솔로에 오르락내리락하는 숨결이 생겨요.
...
펜타토닉에 어제의 B(노랑)에 더해 오늘의 **F(6도, 노랑 강조)** 두 자리가 꼈어요 — 5번 줄 8프렛, 2번 줄 6프렛. 각각 바로 아래 E(5도)로 반음 흘러내리는 다리예요.
...
오늘의 드릴이에요. 1마디는 **F → E(5도)** 반음 해결을 반복하고, 2마디는 **F → E → D → C(♭3)** 로 흘러 홈베이스 C에 착지해요.
...
"Am backing track"을 틀고, 오늘의 미션: **F를 쓸 때마다 반드시 E나 C로 해결하기.** F에서 끝내는 건 금지. 어제의 B(위로 가는 다리)와 오늘의 F(아래로 오는 다리)를 한 프레이즈 안에 같이 써보세요 — 예를 들어 "A-B-C … F-E" 처럼 올라갔다 내려오는 숨결을 만들면 벌써 솔로처럼 들려요.

**40~50분 · 녹음/피드백 (권장)**
아무 녹음 앱으로 잼 30초 녹음. 다시 들으며 딱 하나 체크: **F → E 반음이 "끈적하게" 붙어 들리는지, 아니면 뚝 끊겨 들리는지.** 끊겼으면 손가락을 떼는 타이밍이 이른 거예요. E가 울리기 시작할 때까지 F를 살짝 더 붙잡았다 놓으세요. 필요하면 재생 속도를 늦춰 그 반음 순간을 귀로 확대해서 확인.
