...
오늘 왜 이걸 하냐면요, 어제 "C도 우리 손 안에 있다"는 걸 확인했잖아요? 근데 그냥 확인만 하고 끝나면 A 마이너 하던 관성으로 슬금슬금 돌아가요. 그래서 오늘은 C 메이저에 **확실한 정체성**을 심어줄 거예요. 그 정체성이 바로 **밝은 3도, E**예요.

메이저 사운드가 "밝다"고 느껴지는 이유의 99%는 이 3도(E) 덕분이에요. 마이너는 어두운 b3(C)로 착지해서 쓸쓸하고, 메이저는 밝은 3도(E)로 착지해서 화사하죠. 근데 어제처럼 음을 상하행으로 주르륵 나열하면 이 E가 그냥 "지나가는 음 7개 중 하나"로 묻혀버려요. 그래서 오늘도 무기는 **3도 인터벌**이에요. 스케일에서 바로 옆 음을 건너뛰고 그 다음 음으로 폴짝 뛰는 거요.

3도로 뛰면 마법이 생겨요. C에서 뛰면 E, D에서 뛰면 F, E에서 뛰면 G… 이렇게 **뛰는 착지점마다 코드톤(C·E·G)이 자꾸 걸려요.** 특히 C→E 점프는 그 자체가 "루트에서 밝은 3도로 가는" C 메이저의 핵심 동작이에요. 오늘은 이 C→E 점프를 손이 노래처럼 기억하게 만들고, 마지막엔 E를 길게 늘여 착지하는 감각까지 붙일 거예요.

## ② 시각 자료

C 메이저 포지션이에요. 어제랑 똑같은 지판인데, 오늘은 **C 메이저 관점 도수만** 붙였어요. 루트 **C**와, 오늘의 주인공 밝은 3도 **E**를 강조했어요.
...
오늘의 본체, C 메이저 3도 인터벌 상행 시퀀스예요. C→E, D→F, E→G… 폴짝폴짝 뛰다가 마지막에 밝은 3도 **E**를 길게 늘여 착지해요.
...
**0~10분 · 워밍업 (BPM 84)**
메트로놈 84에 맞춰 3도 인터벌 상행 시퀀스를 6번 줄부터 1번 줄까지. 8분음표 두 개가 "폴-짝" 한 세트예요. C→E 점프가 나올 때마다 살짝 더 또렷하게. 손가락이 꼬이면 80으로 잠깐 내렸다가 다시 올려도 괜찮아요.

**10~20분 · 두뇌 훈련 (오늘의 타겟 = E / 밝은 3도)**
메트로놈 끄고, 지판에서 **E만** 찾아 짚어요. 5번 7프렛, 2번 5프렁 — 이 두 군데. 짚을 때마다 "이-(E)!" 소리 내서 부르고, 바로 옆 4도 F(2번 6프렛)와 헷갈리지 않게 확실히 구분. 그다음 아무 음이나 짚다가 무조건 E로 퇴근하는 걸 3분간. E에 도착할 때마다 "밝다"는 느낌을 의식적으로 각인시켜요.

**20~40분 · 실전 즉흥 (C 메이저 원코드 백킹 / 80~84 BPM)**
"C major backing track" 하나 틀어요. 규칙 하나: **3도로만 뛰어다니고, 프레이즈를 끝낼 땐 반드시 E에 착지.** 붙여 치기 금지, 무조건 한 칸 건너뛰기. E에 착지해서 살짝 비브라토까지 걸어보면 그 화사한 느낌이 확 살아요. 이게 "밝은 3도 착지"의 첫 손맛이에요.

**40~50분 · 녹음/피드백 (권장)**
30초 녹음 후 재생. 체크 포인트: **착지한 E의 음정이 흔들리지 않고 딱 짚였는지**, 그리고 3도 점프에서 중간 음을 실수로 같이 긁지 않았는지. 잡음이 섞이면 피킹 안 하는 현을 오른손 바닥으로 살짝 뮤트하는 습관을.
