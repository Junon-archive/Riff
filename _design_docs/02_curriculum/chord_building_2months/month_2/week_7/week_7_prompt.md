...
- `meta`: `{ "title", "stringCount": 6, "tuning": ["E","A","D","G","B","E"], "key"?, "tempoBpm"?, "notation"? }`
  ...
  - `notation`(선택): `"tab"`(기본, 타브만) | `"staff"`(오선보만) | `"staff+tab"`(오선보+타브를 위아래로 결합 — **박자·리듬을 두 보표가 공유**) | `"rhythm"`(리듬 슬래시). 이번 주 탑노트 멜로디 라인은 `"staff+tab"`으로 렌더해 **음정(오선보)과 프렛·박자(타브)를 동시에** 보이길 권장한다. `type: "tab"` 데이터에만 의미가 있다.
