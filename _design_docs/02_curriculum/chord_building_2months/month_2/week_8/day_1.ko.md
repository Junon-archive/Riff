...
마지막 주에 오신 걸 환영해요! 이번 주 백킹은 R&B/재즈의 심장, **II-V-I**(C키의 **Dm7 → G7 → Cmaj7**) 4마디 루프예요. 이 위에서 2개월간 배운 걸 다 섞어 나만의 컴핑을 만듭니다.

먼저 오늘은 **선택지 카드**를 펼쳐요. 같은 진행을 여러 방식으로 잡을 수 있어요:

- **쉘 보이싱**(4주차) — R·3·7만, 얇고 깔끔. 근음 줄 교차(5·6·5).
- **9도 텐션**(6주차) — 화사한 색을 얹기.
- **고음역·탑노트**(5·7주차) — 얇은 줄로 노래시키기.

정답은 없어요. "이 코드를 지금 어떻게 잡을까?"를 매번 **고르는 것**이 이번 주의 핵심이에요. 오늘은 II-V-I을 쉘로 잡는 기본 세트와, I(Cmaj7)에 9도를 얹는 '선택'을 익힙니다. 부담 갖지 마세요 — 이번 주엔 새로 외울 게 하나도 없어요. 지난 두 달간 손에 차곡차곡 쌓인 도구를 꺼내 쓰는 주간이거든요. 오늘은 그 도구함을 쭉 펼쳐 놓고 "뭐가 있더라?" 둘러보는 날이라고 생각하면 편해요.
...
**선택지: I를 Cmaj7 쉘로, 또는 Cmaj9로.** 9도(노랑)를 얹으면 화사해져요.
...
**0~10분 · 워밍업**
Dm7·G7·Cmaj7 쉘을 각각 잡아 세 줄 또렷이, 버린 줄 죽었는지. 그다음 Cmaj9(9도 추가)도 잡아 봐요.

**10~20분 · 두뇌 훈련 (오늘의 타겟 = 선택지 인지)**
각 코드를 "쉘로? 9도로?" 두 가지로 잡으며, 근음 줄·가이드톤·9도 위치를 매핑. 8주 배운 도구를 머릿속 지도로 정리.

**20~40분 · 실전 반주 (예제 1·2 / 74~84 BPM)**
**예제 1 쉘 II-V-I을 BPM 78에서** 여러 번 → 익으면 **예제 2처럼 I에 9도를 얹어** 봐요. R&B 백킹 트랙(있으면)에 맞춰 두 버전을 번갈아 치며 색을 비교.

**40~50분 · 녹음/셀프 피드백 (권장)**
두 버전 각각 15초 녹음해 비교. 체크: 쉘과 9코드의 색 차이가 들리는가 / 근음 줄 교차가 매끄러운가.
