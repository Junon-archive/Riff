...
今週のゴール。Maj9·9·m9を1つのルート上で循環します。驚きは、**9度（D）が2弦・一番上にずっとそのまま**だということ。変わるのは下の骨組みだけ：

- **3弦＝7度スイッチ。** 4フレット（7、Maj9）↔ 3フレット（b7、9·m9）。
- **4弦＝3度スイッチ。** 2フレット（3、Maj9·9）↔ 1フレット（b3、m9）。
...
**Cmaj9** — 3弦7（4フレット）、4弦3（2フレット）、2弦9（黄色）。
...
```json
{
  ...
  "tab": { "timeSignature": "4/4", "measures": [
    ...
    { "measure": 2, "notes": [
      { "string": 5, "fret": 3, "duration": "quarter", "label": "R", "isRoot": true, "role": "root" },
      { "string": 3, "fret": 3, "duration": "eighth", "label": "b7", "role": "chord_tone" },
      { "string": 2, "fret": 3, "duration": "eighth", "label": "9", "role": "color" },
      { "string": 5, "fret": 3, "duration": "half", "label": "R", "isRoot": true, "role": "root" }
    ]},
    { "measure": 3, "notes": [
      { "string": 5, "fret": 3, "duration": "quarter", "label": "R", "isRoot": true, "role": "root" },
      { "string": 4, "fret": 1, "duration": "eighth", "label": "b3", "role": "chord_tone" },
      { "string": 2, "fret": 3, "duration": "eighth", "label": "9", "role": "color" },
      { "string": 5, "fret": 3, "duration": "half", "label": "R", "isRoot": true, "role": "root" }
    ]}
  ]}
...
**0〜10分 · ウォームアップ**
Cmaj9 → C9 → Cm9 を4拍ずつ順番に。ルート・9度（2弦）は固定、3弦・4弦だけ移動。9度がずっときれいに鳴るかチェック。

**10〜20分 · 脳トレ（今日のターゲット＝固定9度＋骨組みスイッチ）**
「9度はそのまま、7度は4↔3、3度は2↔1」。「m9！」で3弦・4弦を即つかみつつ9度を保持。**例1**で確認。

**20〜40分 · 実戦伴奏（例2循環 / 76〜86 BPM）**
**例2の3つの9コード循環をBPM 80で4回反復。** 続けてルートを移して別キーでも。ソウル/シティポップのバッキングに重ねて華やかな色の進行を楽しんで。

**40〜50分 · 録音・セルフフィードバック＋今週の振り返り（推奨）**
3つの9コード循環を30秒録音。チェック：9度がずっと生きているか／骨組みスイッチで別の弦が死なないか。そして「9度1つで音がどれだけ変わったか」をまとめて。
