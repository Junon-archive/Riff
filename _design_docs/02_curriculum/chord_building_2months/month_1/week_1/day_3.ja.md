...
昨日まで3度で明るさ（メジャー）と暗さ（マイナー）を分けました。今日は**7度**を扱います。正確には**b7（短7度）**。メジャーコードにb7を1つ足すと、**ドミナント7（例：G7）**になります。

ドミナント7の正体は一言で**緊張**です。じっとしているメジャーと違い、b7が入ると「どこかへ行きたい、解決したい」という押し引きの力が生まれます。ブルース・ファンク・ジャズのあの「うごめく」感じは、全部このb7から来ています。

指板では、昨日のGメジャーの**4弦5フレット（R=G）**を**3フレット（b7=F）**へ下げるとG7になります。3度（3弦4フレット、B）はそのまま。まとめると今週の変化はちょうど2軸 — **3弦＝3度（明暗）スイッチ、4弦＝7度（緊張）スイッチ。**
