...
- `id`: `"m{월}.w{주}.d{일}.슬러그"` (예: `"m1.w1.d1.g_major_eform"`).
...
- `meta`: `{ "title", "stringCount": 6, "tuning": ["E","A","D","G","B","E"], "key"?, "tempoBpm"?, "notation"? }`
  - `title`에 코드명·근음 줄·폼 이름을 담는다(예: `"G — 6현 근음(E 폼)"`). ※ 별도 `chord_name` 필드는 쓰지 않는다.
...
```json
{
  ...
  "meta": { "title": "G — 6현 근음(E 폼)", "stringCount": 6, "tuning": ["E","A","D","G","B","E"], "key": "G" },
  "fretboard": {
    "startFret": 3,
    "fretSpan": 4,
    "dots": [
...
```json
{
  "id": "m1.w1.d4.maj_to_min",
  "type": "tab",
  "meta": { "title": "메이저 → 마이너 (3 → b3)", "stringCount": 6, "tuning": ["E","A","D","G","B","E"], "key": "G", "tempoBpm": 70, "notation": "staff+tab" },
  "tab": {
    "timeSignature": "4/4",
    "measures": [
      { "measure": 1, "notes": [
...
- **`role`로 색을 부여한다**(실제 렌더 색 = `color_legend.md`):
  ...
  - '지금 주목할 변화음'(예: 마이너 전환의 b3 착지) → `"role": "target"` + `"highlight": true` → **초록**
...
# 이번 주차 목표 (1개월차 1주차)
**6현 근음(E 폼) 해부, 그리고 메이저 → 마이너 → 7 → m7 손가락 변환 원리 체득.**
핵심 메시지: "코드는 외우는 게 아니라, 6현 루트 위에 3도·5도·b7도를 얹고 손가락 하나를 옮기는 조립 게임이다."

# 4일 커리큘럼 설계 지침
- day_1: 6현 근음 메이저(E 폼)의 해부. 루트(R), 3도, 5도가 지판 어디에 있는지 인터벌로 완전 분해. 근음이 6현에 있다는 감각 각인.
- day_2: 메이저 → 마이너 변환. "3도를 반음 내리면(3 → b3) 밝음이 슬픔으로 바뀐다"는 단 하나의 원리에 집중. 움직이는 손가락 하나를 명확히. (변화음 b3은 `role: "target"` + `highlight`로 강조해 눈에 띄게)
- day_3: 메이저/마이너 → 7(도미넌트7) 변환. b7도를 추가하면 생기는 '긴장감(해결되고 싶은 느낌)'을 설명하고 손가락 배치 변화 제시.
- day_4: m7 완성 및 주간 통합. 마이너에 b7을 더한 m7의 부드러움. 4일간 배운 4가지(Maj, m, 7, m7)를 한 근음 위에서 손가락만 바꿔 전환하는 통합 훈련.

# 데일리 50분 루틴 (③ 오늘의 연습에 반드시 이 타임테이블 적용)
- 0~10분 워밍업: 오늘의 기본 코드 폼 잡기 + 모든 줄이 깔끔하게 울리는지 소리 체크
- 10~20분 두뇌 훈련: 6현 루트를 기준으로 3도·5도·(해당 시)7도의 위치를 지판 위에서 눈 감고 짚어 매핑
- 20~40분 실전 반주: 백킹 트랙을 틀고 오늘 배운 코드로 컴핑. (BPM·키·추천 진행을 구체적으로 지정해 줄 것)
- 40~50분 녹음/셀프 피드백: 스마트폰·PC로 녹음 후 재생. 체크리스트 제공(코드 전환 타이밍이 뭉개지지 않았는가 / 소리 안 나는 줄은 없는가 / 뮤트해야 할 줄이 울리지 않았는가)
