...
어제 두 스위치(2번 줄 3도, 3번 줄 7도)를 배웠죠. 먼저 둘 다 내려 **Cm7**(b3 + b7)을 완성하고, 오늘의 새 손님 **Maj7**을 초대합니다.

핵심은 **3번 줄**에 있어요. 이 줄이 7도의 '삼거리'예요:

- **3번 줄 5프렛 = R**(옥타브 근음) → 그냥 메이저/마이너
- **3번 줄 4프렛 = 7**(장7도) → **Maj7**
- **3번 줄 3프렛 = b7**(단7도) → **도미넌트 7**

즉 **b7(3프렛)과 Maj7의 7(4프렛)은 딱 한 프렛, 반음 차이**예요. 그런데 이 반음이 완전히 다른 감정을 만들어요. b7(도미넌트)은 "빨리 해결되고 싶어" 하는 긴장이고, Maj7의 7은 "떠 있는, 몽환적이고 세련된" 미완이에요. 재즈·시티팝의 그 아련한 색이 바로 이 Maj7이에요. 오늘은 이 둘을 3번 줄에서 나란히 놓고 귀로 구분합니다.

## ② 시각 자료

먼저 두 스위치를 다 내린 **Cm7(A 폼)** — 초록이 b3와 b7이에요.
...
오늘의 새 코드 **Cmaj7(A 폼)** — 3번 줄이 b7(3프렛)이 아니라 **7(4프렛)**. 딱 한 칸 위예요(초록 = 7).
...
**0~10분 · 워밍업**
3번 줄만 5(R)→4(Maj7)→3(b7)으로 오르내리며 세 갈래를 손에 익혀요. 나머지 손가락은 고정.

**10~20분 · 두뇌 훈련 (오늘의 타겟 = b7 vs 7)**
**예제 1**로 b7(3프렛)과 7(4프렛)을 번갈아 짚으며 소리를 비교. 눈 감고 "7! b7!" 지시대로 3번 줄에서 바로 짚으면 통과.

**20~40분 · 실전 반주 (예제 2 컴핑 / 75~85 BPM)**
**예제 2를 BPM 80에서 4번 반복**(C7↔Cmaj7). 이어서 Cm7까지 넣어 Cm7 → C7 → Cmaj7을 순환해 보세요. 근음만 5번 줄에서 옮겨 다른 키로도.

**40~50분 · 녹음/셀프 피드백 (권장)**
b7 ↔ 7 전환 30초 녹음. 체크: 두 코드가 **감정적으로 다르게** 들리는지, 3번 줄 음이 또렷한지.
