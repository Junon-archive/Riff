...
先週はずっとルートが**6弦**にありました。今週はなぜその家を**<mark>5弦</mark>**へ移すかというと — ルート弦を1つ自由にするだけで、指板全体がぐっと開くからです。安心して、ルールは1つも変わりません。相変わらず「ルートの上に3度・5度を乗せる」です。ただルートが別の弦へ行くと、3度・5度も新しい場所へ付いて移るので、今日は<mark>その新しい地図を目に馴染ませる日</mark>です。
...
- **3（3度）** — 2弦5フレット（E）。<mark>今日の表情担当</mark>（緑）。

そして**6弦は使いません。** ここでAフォームの新しい習慣が1つ生まれます — **<mark>6弦を確実に殺すこと</mark>**。ルートより低い弦が漏れて鳴ると<mark>コードが濁ります</mark>。親指で軽く覆うか、ルートを押さえる指先で6弦に触れてミュートして。最初は気にすることが1つ増えたようでも、数日で手が勝手にやってくれます。

## ② ビジュアル

5弦にルートを置いた**Cメジャー（Aフォーム）**。6弦・1弦は✕（ミュート）、緑が2弦の**<mark>3度（E）</mark>**です。
...
**0〜10分 · ウォームアップ**
C（Aフォーム）を押さえ、5弦から2弦まで1本ずつ鳴らします。最優先チェックは**<mark>6弦と1弦が確実に死んでいるか</mark>**。使う4本だけきれいに。

**10〜20分 · 脳トレ（今日のターゲット＝3度E）**
**例1**を押さえてR・3・5を声に出し、2弦5フレットの**<mark>3度（E）</mark>**だけを何度もピンポイントで。ルートが5弦のとき3度がどこへ行ったかを目を閉じて押さえられれば合格。

**20〜40分 · 実戦伴奏（例2コンピング / 70〜80 BPM）**
**例2をBPM 75で4回反復。** ストロークのたびに6弦が死んでいるか意識。慣れたらルートだけ5弦5フレット（D）に移して同じパターンを応用。今日は「ルートが5弦」という<mark>新しい座標に手を慣らす日</mark>。

**40〜50分 · 録音・セルフフィードバック（推奨）**
Cコンピングを30秒録音。チェック：**<mark>低音6弦が漏れて鳴っていないか</mark>**、4本がはっきりしているか。6弦が鳴ったらミュート指の位置を明日修正。
