...
先週6弦ルートで学んだ2つの原理を、5弦ルートでそのまま使います。新しく覚えるのはただ1つ、**どの弦でスイッチするか**だけ。

- **マイナースイッチ＝2弦。** 3度（2弦5フレット、E）を半音下げてb3（4フレット、E♭）。→ **Cm**
- **ドミナント7スイッチ＝3弦。** オクターブルート（3弦5フレット、C）を2フレット下げてb7（3フレット、B♭）。→ **C7**

6弦フォームでは3度が3弦、7度が4弦でした。5弦フォーム（Aフォーム）では**3度が2弦、7度が3弦**へ引っ越し。ルートが変わればスイッチの場所も変わる — これを手で確認するのが今日のポイント。原理（3度を下げればマイナー、b7を足せば7）は先週と100％同じです。
