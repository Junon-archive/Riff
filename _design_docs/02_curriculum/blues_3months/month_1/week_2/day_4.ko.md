...
2주차의 결승선이에요. 오늘은 지금까지의 조각을 모두 합쳐요. **12마디 폼**(D3) 위에, **옮긴 부기**(D2)를 얹어 한 바퀴를 완성해요. <mark>코드가 바뀔 때마다 부기의 루트 줄만 5→4→6번으로 갈아타면 돼요.</mark> 새로 배우는 건 없어요. 이미 손에 있는 것들을 연결할 뿐이에요.

오늘의 핵심은 **이동**이에요. 부기 손모양은 그대로 두고, 코드가 바뀌는 순간 **루트 줄만 한 줄 위나 아래로** 옮겨요. 예를 들어 1마디 A7에서 2마디 D7으로 갈 때, 5번 줄에서 걷던 손이 4번 줄로 통째로 올라가요. <mark>손가락 간격은 하나도 안 바뀌어요.</mark>
...
오늘 완성할 대표 이동은 **A7→D7**이에요. 이 두 마디가 매끄러우면 나머지 코드 이동도 똑같은 원리라 금방 이어져요.
