...
- 핵심 문장·단어 `<mark>…</mark>` → **베이스 페이지에서 연한 초록** 자동 렌더(저자는 `<mark>`만). 문단당 1~2곳.
...
# [필수] 오늘의 완성물 명시
- 각 day 끝 **"오늘의 완성물:"**. **day_4**엔 **"이번 주 완성물:"**(R-5-8 라인 E-C-G-D를 4·5현 녹음) + **"1개월 졸업 작품"**임을 명시하고, 4주 성장 회고 + 다음 트랙 안내로 마무리한다.
