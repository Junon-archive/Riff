...
- **day_2**: **옥타브 추가**(옥타브 박스 = 루트 +2현 +2프렛, 줄 건너뛰기). 완성: R-5-8.
  - **★지판 필수:** 옥타브 박스는 시간순 tab만이 아니라 **`type:"fretboard_diagram"`(손모양)로도** 병기한다(4·5현 `_4`/`_5`). 루트·5도·옥타브 3점을 dots로 찍어 "2현 위+2프렛 위" 손모양이 눈에 각인되게 — 옥타브 label "8", 근음 role:root, 5도·옥타브 role:chord_tone(파랑).
