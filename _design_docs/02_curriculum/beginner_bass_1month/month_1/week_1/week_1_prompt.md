...
- 산문(①③④)에서 핵심 문장·단어를 `<mark>…</mark>`로 감싼다. **베이스 페이지에선 이 하이라이트가 연한 초록색**으로 렌더된다(기타=하늘색과 다른 악기 액센트). **저자는 `<mark>`만 쓰면 된다** — 색은 페이지 테마가 자동 적용. 한 문단 1~2곳, 남발 금지.
...
- **17~27분 · 적용:** 오늘 기술을 위 예제/펄스에 얹어 반복(**오늘의 완성물**). BPM 구체 지정.
- **27~30분 · 체크:** 메트로놈 BPM 기록, 원하면 30초 녹음해 버징/잡음 확인.

# [필수] 오늘의 완성물 명시
- 각 day 끝에 **"오늘의 완성물:"** 한 줄. **day_4**엔 **"이번 주 완성물:"**(E·A 루트 펄스를 60~65에서 4·5현 둘 다).
