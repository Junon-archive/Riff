...
- **day_1**: 베이스와 인사 — 자세·스트랩·튜닝 + **악기 파트(4현 vs 5현이 뭔지, 저음 B 줄의 존재)** + **오른손 투핑거**(검지-중지 교대). 개방 E를 고르게. 완성: 개방 E 8분음표 균일.
  - **★지판 필수:** 4현/5현의 각 현 위치(E·A·D·G, 5현은 +저음 B)를 `type:"fretboard_diagram"`로 4·5현 병행(`_4`/`_5`) 지도로 보여준다 — "어느 베이스든 E·A·D·G 위치는 같다"가 한눈에 오게. dots에 각 현 개방(fret 0) + 현이름 label, 근음 E는 role:root(파랑).
