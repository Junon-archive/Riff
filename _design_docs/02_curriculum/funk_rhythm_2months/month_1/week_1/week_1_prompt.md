...
- `meta`: `{ "title", "stringCount": 6, "tuning": ["E","A","D","G","B","E"], "key"?, "tempoBpm"?, "notation"? }`
  ...
  - `notation`(선택): `"tab"`(기본, 타브만) | `"staff"`(오선보만) | `"staff+tab"`(오선보+타브를 위아래로 결합 — **박자·리듬을 두 보표가 공유**). **펑크는 리듬이 핵심**이므로 16비트 그루브·스크래치·컴핑 패턴은 모두 `"staff+tab"`으로 내서 박자를 오선보로 보이게 한다. ⛔ `"rhythm"`(리듬 슬래시)은 현재 렌더러 미지원이므로 쓰지 않는다. `type: "tab"` 데이터에만 의미가 있다.
