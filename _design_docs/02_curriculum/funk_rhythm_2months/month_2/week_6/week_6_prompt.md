...
5. **연습용 예제 릭/그루브를 반드시 포함한다(개념 설명만으로 끝내지 말 것).** 각 day 마다 그날 배운 라인·리듬·테크닉을 **실제로 쳐볼 수 있는 구체 예제**를 최소 2개, `type:"tab"` 표준 JSON으로 제시한다.
   ...
   - 리듬/커팅이 섞이는 날 → `meta.notation:"staff+tab"`으로 그루브 패턴 1개 이상. 소리 없는 자리는 `rest`, 뮤트 타격·고스트는 `technique:"dead_note"`.
...
- `meta`: `{ "title", "stringCount": 6, "tuning": ["E","A","D","G","B","E"], "key"?, "tempoBpm"?, "notation"? }`
  ...
  - `notation`(선택): `"tab"` | `"staff"` | `"staff+tab"`(오선보+타브 결합, **박자 공유**). **싱글 노트 라인은 음정+리듬을 함께** 보여줘야 하므로 그루브·라인은 모두 `"staff+tab"`으로 낸다. ⛔ `"rhythm"`(리듬 슬래시)은 현재 렌더러 미지원이므로 쓰지 않는다. `type: "tab"` 데이터에만 의미가 있다.
...
- 싱글 노트 라인·리듬 예제는 `type:"tab"` + `meta.notation:"staff+tab"`(음정+박자 동시).
- **실제로 울리는 음** = `notes[]` 항목으로 표기한다. 이번 주 **싱글 노트 라인·고스트 단타는 단음으로** 둔다(화음으로 합치지 않음) — 한 줄 중심의 베이스라인식 라인이므로 string/fret을 정확히 지정한다. 코드 스트로크(동시타)가 섞일 땐 **`chord[]`로 쌓는다** — 대표음은 그 화음의 **최저음**(string 번호 최대)을 `string`/`fret`에, 나머지 음은 `chord[]`에 담고 각 음의 `role`은 같은 날 지판(fretboard) dot과 맞춘다(`chord[]`는 마디 박자합에 기여하지 않음). `stroke`는 아르페지오(펼침)에만 쓰고, 동시 스트럼은 `stroke` 없이 화음 표기만으로 나타낸다.
